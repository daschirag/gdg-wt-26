...
impl Pipeline {
    ...
    pub fn execute(
        ...
        let (sst_rate, row_rate) =
            crate::aqp::accuracy::AccuracyCalculator::split_sampling_rate(sampling_rate);
        overall_profile.sst_sampling_ms = start_sampling.elapsed().as_secs_f64() * 1000.0;

        let mut warnings = Vec::new();
        if sampling_rate < 0.05 {
            warnings.push(format!(
                "Sampling rate clamped to 0.05 (calculated: {})",
                (1.0 - config.accuracy_target).powf(config.k)
            ));
        }
        ...
        let sst_with_counts: Vec<(String, u64)> = filtered_sstables
            .iter()
            .map(|path| {
        ...
        let sampled_sst_paths_with_counts: Vec<(String, u64)> = sampled_sst_paths
            .into_iter()
            .filter_map(|p| sst_with_counts.iter().find(|(path, _)| *path == p).cloned())
            .collect();
        ...
        let needed_indices: Vec<usize> = required_cols
            .iter()
        ...
        let mut final_i64: std::collections::HashMap<usize, AlignedVec<i64>> =
            std::collections::HashMap::new();
        let mut final_f64: std::collections::HashMap<usize, AlignedVec<f64>> =
            std::collections::HashMap::new();
        ...
        for (path, _) in &sampled_sst_paths_with_counts {
            if let Ok(reader) = ColumnarReader::new(path.into(), 0) {
                ...
                let mut seg_i64: std::collections::HashMap<usize, AlignedVec<i64>> =
                    std::collections::HashMap::new();
                let mut seg_f64: std::collections::HashMap<usize, AlignedVec<f64>> =
                    std::collections::HashMap::new();
    ...
    fn apply_filter_columnar_indexed(
        &self,
